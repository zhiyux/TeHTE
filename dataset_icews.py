...
class Dataset_ICEWS():
    def __init__(self, args):
        ...
        self.build_dict_test_valid(base_dir)
    ...
    #             self.get_id(h, self.ent2id)
    #             self.get_id(t, self.ent2id)
    def build_dict_test_valid(self, base_dir):
        path = os.path.join(base_dir, '{}.txt'.format('test'))
        # cn = 0
        with open(path, 'r') as f:
            for line in f.readlines():
                h, r, t, d = line.split('\t')
                self.get_id(h, self.ent2id)
                self.get_id(t, self.ent2id)
        path = os.path.join(base_dir, '{}.txt'.format('valid'))
        # cn = 0
        with open(path, 'r') as f:
            for line in f.readlines():
                h, r, t, d = line.split('\t')
